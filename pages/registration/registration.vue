	...
	<view class="registrationLayout pageBg">
		...
		<view class="wraper">
			<wd-sidebar v-model="active" @change="handleChange" style="width: 250rpx;">
				<wd-sidebar-item style="width: 250rpx;" v-for="(item, index) in categories" :key="index" :value="index"
					:label="item.label" :icon="item.icon" :disabled="item.disabled" />
			</wd-sidebar>
			<view class="content" :style="`transform: translateY(-${active * 100}%)`">
				<scroll-view v-for="(item, index) in categories" :key="index" class="category" scroll-y
					...
					<wd-cell-group :title="item.title" border>
						<wd-cell v-for="(cell, index) in item.items" :key="index" :title="cell.title"
							:label="cell.label" is-link :to="`/pages/doctor/doctor?specializationId=${cell.id}`">
